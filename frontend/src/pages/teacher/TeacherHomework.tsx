...
import { Users, BookOpen, ArrowRight, ShieldAlert } from 'lucide-react';

const TeacherHomework = () => {
  const navigate = useNavigate();

  const { data: groups = [], isLoading } = useQuery({
    queryKey: ['teacher-homework-groups'],
    queryFn: async () => {
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center">
          ...
          Uy vazifalarini boshqarish
        </h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">Uy vazifasi yuborish va tekshirish uchun quyidagi guruhlardan birini tanlang</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {groups.map((group: any) => (
            ...
            >
              <div>
                <span className="px-3 py-1 bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 text-xs font-bold rounded-full">
              ...
              <button 
                onClick={() => navigate(`/teacher/groups?groupId=${group.id}&tab=homework`)}
                className="mt-6 w-full py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-2xl font-bold flex items-center justify-center transition-all group shadow-md shadow-orange-500/5"
              >
